...
class autoencoder_nonet2nd_thz_randinit(autoencoder_nonet2nd):
    # -----------------------------------------------------------------------
    def __init__(self, dataset_name, dataset_filename, dir_data, dir_dest, hp, verbose = True):
        super(autoencoder_nonet2nd_thz_randinit, self).__init__(dataset_name, dataset_filename, dir_data, dir_dest, hp, verbose)
        ...
        # load my model
        self.model = model_thz(device = self.device, dataset = self.dataset, SHOULD_PROJECT_BEFORE = hp.SHOULD_PROJECT_BEFORE, verbose = verbose)
        ...
        if self.verbose:
            print("autoencoder_nonet2nd_thz_randinit.init")

    # -----------------------------------------------------------------------
    def get_network_name(self):
        return "nonet2nd_randinit"