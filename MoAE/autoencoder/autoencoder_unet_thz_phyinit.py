...
class autoencoder_unet_thz_phyinit(autoencoder_unet):
    # -----------------------------------------------------------------------
    def __init__(self, dataset_name, dataset_filename, dir_data, dir_dest, hp, verbose = True):
        super(autoencoder_unet_thz_phyinit, self).__init__(dataset_name, dataset_filename, dir_data, dir_dest, hp, verbose)
        ...
        if self.verbose:
            print("autoencoder_unet_thz_phyinit.init")
    ...
    def train(self):
        ...
        while run <= self.RUNS:
            ...
            with torch.no_grad():
                N_init = self.network(batch_reshape).unsqueeze(1).to(self.device).detach().requires_grad_(False)
                u_init = self.model.initialize_u(batch = self.dataset[0], use_logit = True).to(self.device).detach().requires_grad_(False)
            ...
            while epoch <= self.hp.EPOCHS and self.EPOCH_CONTINUE:
                ...
                def closure():
                    ...
                    self.model_predict = self.model.forward_with_init(batch_shape = self.dataset[0].shape, 
                    ...
                    loss = self.loss(self.model_predict, self.dataset[0])
